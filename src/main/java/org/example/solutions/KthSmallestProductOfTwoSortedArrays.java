...
public class KthSmallestProductOfTwoSortedArrays {
    public long kthSmallestProduct(int[] nums1, int[] nums2, long k) {
        ...
        while (left <= right) {
            ...
            for (int i = 0; i < nums1.length; i++) {
                count += countProducts(nums2, nums1[i], mid);
            }
            if (count < k) {
                left = mid + 1;
            } else {
    ...
    public long countProducts(int[] arr, int val, long product) {
        int left = 0;
        int right = arr.length - 1;
        while (left <= right) {
            int mid = left + (right - left) / 2;
            long currentProduct = (long) val * (long) arr[mid];
            if ((val >= 0 && currentProduct <= product) || (val < 0 && currentProduct > product)) {
                left = mid + 1;
            } else {
                right = mid - 1;
            }
